...
const FIXTURES = [
  ...
  ['F246', 'every un-sendable artifact stays outside the sendable directory, and the label names it', () => {
    ...
    const saltDir = path.join(root, 'salt');
    writeCorpus(root);
    // The declaration gate is upstream of every artifact this fixture looks
    // for: unanswered, the export refuses before the candidates file exists.
    declareValues(saltDir);
  ...
  // F251-F254. known-values.json was optional, and README called no file "the
  ...
  ['F251', 'an export with no known-values.json and no acknowledgement produces no archive', () => {
    const root = tmpdir();
    const out = path.join(root, 'out');
    const saltDir = path.join(root, 'salt');
    writeCorpus(root);
    ...
    assert.equal(
      archivesIn(out).length + fs.readdirSync(out).filter((f) => f.endsWith('.zip')).length,
      0,
  ...
  ['F252', 'the acknowledged path exports, and the manifest says the operator declared nothing', () => {
    const root = tmpdir();
    const out = path.join(root, 'out');
    const saltDir = path.join(root, 'salt');
    writeCorpus(root);
    ...
    assert.equal(archivesIn(out).length, 1, 'no archive was written');
    assert.match(
      done.out,
  ...
  ['F253', 'a run that declared nothing and a run that declared are different facts in the machine document', () => {
 ...
 'no personal value is ever inferred from the machine to fill the declaration', () => {
  ...
  ['F258', 'nothing decides from process.platform what the filesystem can be asked', () => {
